...
import torchvision
## 创建文件夹
os.makedirs("./images/gan6/", exist_ok=True)  ## 记录训练过程的图片效果
os.makedirs("./save/gan6/", exist_ok=True)  ## 训练完成时模型保存的位置

parser = argparse.ArgumentParser()          #命令行选项、参数和子命令解析器
parser.add_argument("--n_epochs", type=int, default=100, help="number of epochs of training")  #迭代次数
parser.add_argument("--batch_size", type=int, default=128, help="size of the batches")          #batch大小
...
parser.add_argument("--n_cpu", type=int, default=2, help="number of cpu threads to use during batch generation") #CPU个数
...
parser.add_argument("--sample_interval", type=int, default=500, help="interval between image sampling")  #保存图像的迭代数
...
# Loss function
adversarial_loss = Focal_Loss()       #定义了一个BCE损失函数
...
for epoch in range(opt.n_epochs):
    for i, (imgs, _) in enumerate(dataloader):
        ...
        z = Variable(Tensor(np.random.normal(0, 1, (imgs.shape[0], opt.latent_dim))))#生成的噪音 随机构00维向量 均值0方差1维度(128，100)的噪音，随机初始化一个128大小batch的向量
        ...
        if batches_done % opt.sample_interval == 0:
            save_image(gen_imgs.data[:25], "images/gan6/%d.png" % batches_done, nrow=5, normalize=True)
## 保存模型
torch.save(generator.state_dict(), './save/gan/generator6.pth')
torch.save(discriminator.state_dict(), './save/gan/discriminator6.pth')