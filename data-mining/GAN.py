...
## 创建文件夹
os.makedirs("./images/gan2/", exist_ok=True)  ## 记录训练过程的图片效果
...
# 加载图片数据集
dataset = torchvision.datasets.ImageFolder(root='E:/GAN1/face', transform=transform)
print(dataset)
# 创建数据加载器
dataloader = DataLoader(dataset, batch_size=opt.batch_size, shuffle=True)

## ##### 定义判别器 Discriminator ######
## 将图片28x28×3展开成2352，然后通过多层感知器，中间经过斜率设置为0.2的LeakyReLU激活函数，
## 最后接sigmoid激活函数得到一个0到1之间的概率进行二分类
class Discriminator(nn.Module):
    def __init__(self):
        super(Discriminator, self).__init__()
        self.model = nn.Sequential(
            nn.Linear(img_area, 512),  ## 输入特征数为2352，输出为512
    ...
    def forward(self, img):
        img_flat = img.view(img.size(0), -1)  ## 鉴别器输入是一个被view展开的(2352)的一维图像:(128, 2352)
...
## 然后经过线性变换将其变成2352维，最后经过Tanh激活函数是希望生成的假的图片数据分布, 能够在-1～1之间。
class Generator(nn.Module):
    def __init__(self):
        ...
        ## prod():返回给定轴上的数组元素的乘积:3*28*28=2352
        self.model = nn.Sequential(
            ...
            *block(512, 1024),  ## 线性变化将输入映射 512 to 1024, 正则化, LeakyReLU
            nn.Linear(1024, img_area),  ## 线性变化将输入映射 1024 to 2352
            nn.Tanh()  ## 将(2352)的数据每一个都映射到[-1, 1]之间
        )

    ## view():相当于numpy中的reshape，重新定义矩阵的形状:这里是reshape(128, 3, 28, 28)
    def forward(self, z):  ## 输入的是(128， 100)的噪声数据
        imgs = self.model(z)  ## 噪声数据通过生成器模型
        imgs = imgs.view(imgs.size(0), *img_shape)  ## reshape成(128, 3, 28, 28)
        return imgs  ## 输出为64张大小为(3, 28, 28)的图像
...
for epoch in range(opt.n_epochs):  ## epoch:50
    for i, (imgs, _) in enumerate(dataloader):  ## imgs:(128, 3, 28, 28)     _:label(128)

        ## =============================训练判别器==================
        ## view(): 相当于numpy中的reshape，重新定义矩阵的形状, 相当于reshape(128，784)  原来是(128, 3, 28, 28)
        imgs = imgs.view(imgs.size(0), -1)  ## 将图片展开为28*28=784  imgs:(128, 784)
...
## 保存模型
torch.save(generator.state_dict(), './save/gan/generator2.pth')
torch.save(discriminator.state_dict(), './save/gan/discriminator2.pth')
